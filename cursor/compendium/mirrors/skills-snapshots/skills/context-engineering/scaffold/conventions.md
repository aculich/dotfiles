...
- **project_slug:** TODO
- **repo_root:** `.`
- **task_runner:** `just` — add repo-root **`justfile`** (default recipe = read-only **`status`**) and **`docs/AUTOMATION.md`** explaining **just** vs Cursor slash skills vs **`cursor-agent`** / **`claude`** (umbrella repos).
