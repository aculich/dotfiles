...
description: >-
  Normalizes a long-lived client umbrella repo (engagements/, .context/,
  incoming/ triage, justfile + docs/AUTOMATION.md) and folds nested git via
  subtree. Existing repos: dry-run first; apply only after umbrella bootstrap
  apply / explicit approve. Pairs with context-engineering, meeting-sync,
