...
- **Agents and multi-root** — Cursor’s changelog describes **multi-root workspaces** in the **Agents** window so a session can target **more than one folder** (e.g. frontend + backend + shared lib) without re-adding folders every turn. [Cursor: Multitask, Worktrees, and Multi-root Workspaces (2024-04-26)](https://cursor.com/changelog/04-24-26). To open the **classic IDE** instead of Agents Window, use the `--classic` wrapper: [cursor-classic-ide.md](cursor-classic-ide.md).
