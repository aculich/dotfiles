...
· Export agent sessions from all IDEs and CLIs (e.g., Cursor, VSCode, Claude Code, Gemini CLI, Codex, Cursor CLI, Factory Droid CLI), not just the current IDE.
· Add support for remote workspaces (SSH, WSL, dev containers).
· Add Windows support.
· Improve export speed by using the new Go-based CLI.
· Bugfix: Fix an issue where Copilot sessions were not being exported correctly.

## [0.32.3] - 2026-03-03

· Bugfix: Load cursor conversation data in batches to avoid SQLite expression tree depth limits

## [0.32.2] - 2026-02-17

· Bugfix: Attempt to fix some more trimming issues in agent responses

## [0.32.1] - 2026-02-16

· Bugfix: Fix an issue with the auto-save service not properly escaping shell characters in the composer filename.

## [0.32.0] - 2026-02-16

· Use applyTo instead of globs in the AI rules file

## [0.31.1] - 2026-02-03

· Bugfix: Fix an issue when reading JSON composer files in VSCode

· Bugfix: Attempt to fix an issue when trimming some tool invocation responses

## [0.31.0] - 2026-02-03

- Configurable history folder path via `specstory.outputPath` setting (defaults to `.specstory/history`)

## [0.30.0] - 2026-01-30

· Add support for the new VS Code conversations storage format
